...
func TestFlatMap(t *testing.T) {
	f := func(v string) gs.Either[int, int64] {
		...
		if err != nil {
			return impl.Left[int, int64](0)
		}
		return impl.Right[int, int64](a)
	}

	r := impl.Right[int, string]("1000")
...
func TestMap(t *testing.T) {
	...
	*/
	...
	e := either.Map(impl.Right[int, int](12), f)
	assert.Equal(t, true, e.IsRight())
	assert.Equal(t, "flower", e.Right())

	e = either.Map(impl.Left[int, int](12), f)
	assert.Equal(t, true, e.IsLeft())
